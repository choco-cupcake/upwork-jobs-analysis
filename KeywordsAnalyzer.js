...
const filterKeywords = ['frontend', 'backend', 'database', 'evm', 'smart contract', 'full stack', 'full-stack', 'audit', 'solidity', 'nft', 'blockchain', 'python', 'solana', 'rust', 'golang', 'bitcoin']
const settings = {
    ...
    MIN_PRICE_AMOUNT_RECORDINGS: 10,
}
...
async function main(){
    ...
    //normalizedPriceAmount = normalizedPriceAmount.filter(e => e.priceRecordings > settings.MIN_PRICE_AMOUNT_RECORDINGS)
    ...
    if(collection == 'all'){
        ...
        for(let k of searchKeywords){
            ...
            for(let j of jobs){
                let _new = true
                for(let r of ret)
                    if(r.description == j.description){
                        _new = false
                        break
                    }
                if(_new) ret.push(j)
...
function craftBOW(job){
    ...
    for(let mwk of settings.multiWordKeywords){
        let occurrences = text.matchAll(mwk).length
        if(occurrences){
    ...
    let words = text.split(' ').filter(e => e.length >= 4)
    for(let w of words){
...
function fillKeywordExtras(keyword, job){
    if(settings.analyzePrices){
        ...
        if(job.amount && job.amount.currencyCode == 'USD' && job.amount.amount != 0){
